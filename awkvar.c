...
int awkvar(char *config[])
{
	...
	sprintf(sys_command,"mkdir %s/Variants",current_directory);
	system(sys_command);
	sprintf(sys_command,"mkdir %s/Variants/logs",current_directory);
	system(sys_command);
	
	/* Go through each of the lines in the filelist.txt file and awk out the variants into the Variants Directory */
	filelistfp = fopen(config[0],"r");
	if (filelistfp == NULL) {
	...
	while (fgets(filestring,50000,filelistfp) != NULL) {
		...
		printf("filename contains %s\n",filename);
		sprintf(snplist_job_file,"%s/Variants/logs/snplistJob%d.job",config[1],i);
		...
		if (logp == NULL) {
			...
			fprintf(logp, "#BSUB -J snplistJob%d\n", i);
			fprintf(logp, "#BSUB -o %s/Variants/logs/snplistJob%d.out\n",config[1],i);
			fprintf(logp, "#BSUB -e %s/Variants/logs/snplistJob%d.err\n",config[1],i);
			...
			fprintf(logp, "awk '/^#/ || $5 != \".\" ' %s > %s/Variants/%s\n\n",filestring,config[1],filename);

		}
		fclose(logp);
		printf("file is %s/Variants/logs/snplistJob%d.job\n",config[1],i);
		sprintf(sys_command,"bsub < %s/Variants/logs/snplistJob%d.job\n",config[1],i);
		system(sys_command);
